...
class FlowdockFormatterTest extends TestCase
{
    ...
    {
        $formatter = new FlowdockFormatter('test_source', 'source@composer.com');
        $record = $this->getRecord();

        $expected = array(
            'source' => 'test_source',
            'from_address' => 'source@composer.com',
            'subject' => 'in test_source: WARNING - composer',
            'content' => 'composer',
            'tags' => array('#logs', '#warning', '#composer'),
            'project' => 'test_source',
        );
