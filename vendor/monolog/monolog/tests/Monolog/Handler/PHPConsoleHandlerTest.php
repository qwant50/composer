...
class PHPConsoleHandlerTest extends TestCase
{
    ...
    {
        return new Logger('composer', array(
    ...
    {
        $this->debugDispatcher->expects($this->once())->method('dispatchDebug')->with($this->equalTo('composer'));
        $this->initLogger()->addDebug('composer');
    }
    ...
    public function testDebugTags($tagsContextKeys = null)
    {
        ...
        foreach ($tagsContextKeys as $key) {
            ...
            $logger->addDebug('composer', array($key => $expectedTags));
        }
    }
    ...
    public function testOptionEnabled()
    {
        ...
        $this->initLogger(array('enabled' => false))->addDebug('composer');
    }
